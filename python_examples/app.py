...
clr.AddReference("C:\\Program Files\\Bruel and Kjaer\\EA Engine\\Signal Processing Library.dll")
        ...
        if args.FrequencyAxis.GetLength(0) != 0:
            ...
            for i in range(0, args.FrequencyAxis.GetLength(0)):
                if (args.FrequencyAxis[i] >= 500) and (args.FrequencyAxis[i] <= 1500):
                    ...
                    if unit == "dB":
                        y2.append(20 * np.log10(args.ResponseAutoSpectrum[i] / 2e-5))
                    else:
...
class Window(QMainWindow, Ui_Form):
    ...
    def tabChanged(self):
        ...
        if self.tabWidget.currentIndex() == 1:
            ...
            if calibrationsettings.ResultFileFormatType == 0:
                self.comboBox_ResultsFormat.setCurrentText("XML")
            if calibrationsettings.ResultFileFormatType == 1:
                self.comboBox_ResultsFormat.setCurrentText("MATLAB")
        ...
        if self.tabWidget.currentIndex() == 2:
            ...
            outputchannels[0].SignalType = GeneratorSignalType.White
            outputchannels[0].Random.Slope = 0
            engine.SetOutputChannels(outputchannels)

            self.lineEdit_Level_RandomNoiseTest.setText(str(outputchannels[0].Random.Level))
        ...
        if self.tabWidget.currentIndex() == 3:
            ...
            outputchannels[0].SignalType = GeneratorSignalType.Step
            engine.SetOutputChannels(outputchannels)

            self.lineEdit_Level_StepSineTest.setText(str(outputchannels[0].StepSine.Level))
            ...
            if outputchannels[0].StepSine.ResolutionType == StepSineResolutionType.R10:
                self.comboBox_Resolution_StepSineTest.setCurrentText("R10")
            if outputchannels[0].StepSine.ResolutionType == StepSineResolutionType.R20:
                self.comboBox_Resolution_StepSineTest.setCurrentText("R20")
            if outputchannels[0].StepSine.ResolutionType == StepSineResolutionType.R40:
                self.comboBox_Resolution_StepSineTest.setCurrentText("R40")
            if outputchannels[0].StepSine.ResolutionType == StepSineResolutionType.R80:
    ...
    def selectDevice(self):
        ...
        if idx != -1:
            if self.checkBox_Reset.isChecked():
                device = engine.SelectDevice(idx + 1, True)
            else:
                device = engine.SelectDevice(idx + 1, False)
    ...
    def saveChannels(self):
        for i in range(0, self.tableWidget_Inputs.rowCount()):
            if self.tableWidget_Inputs.item(i, 0).checkState() == 0:
                ...
                active = True
            engine.SetInputChannel(i + 1, self.tableWidget_Inputs.item(i, 1).text(), active,
                                   self.tableWidget_Inputs.item(i, 2).text(), self.tableWidget_Inputs.item(i, 3).text(),
                                   self.tableWidget_Inputs.item(i, 4).text(), self.tableWidget_Inputs.item(i, 6).text(),
                                   self.tableWidget_Inputs.item(i, 8).text())
        ...
        for i in range(0, self.tableWidget_Outputs.rowCount()):
            if self.tableWidget_Outputs.item(i, 0).checkState() == 0:
                ...
                active = True
            engine.SetOutputChannel(i + 1, self.tableWidget_Outputs.item(i, 1).text(), active,
                                    self.tableWidget_Outputs.item(i, 2).text(),
                                    self.tableWidget_Outputs.item(i, 3).text(),
                                    self.tableWidget_Outputs.item(i, 4).text(),
                                    self.tableWidget_Outputs.item(i, 6).text(),
                                    0)
    ...
    def inputCalibration(self):
        ...
        calibrationSettings.ReferenceFrequency = self.lineEdit_RefFrequency.text()
        calibrationSettings.ReferenceLevel = self.lineEdit_RefLevel.text()
        calibrationSettings.ReferenceUnit = self.comboBox_RefLevelUnit.currentText()
        calibrationSettings.Duration = self.lineEdit_Duration_InputCalibration.text()
        ...
        calibrationSettings.AnalysisFFTSettings.BlockSize = self.comboBox_FFTBlocksize.currentText()
    ...
    def randomNoiseLevel(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].Random.Level = self.lineEdit_Level_RandomNoiseTest.text()
        engine.SetOutputChannels(outputchannels)
    ...
    def randomNoiseFmin(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].Random.HiPassFrequency = self.lineEdit_Fmin_Output_RandomNoiseTest.text()
        engine.SetOutputChannels(outputchannels)

    def randomNoiseFmax(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].Random.LoPassFrequency = self.lineEdit_Fmax_Output_RandomNoiseTest.text()
        engine.SetOutputChannels(outputchannels)
    ...
    def randomNoiseSignalType(self):
        ...
        if self.comboBo_SignalType.currentText() == "Pink":
            outputchannels = engine.GetOutputChannels(False)
            outputchannels[0].SignalType = GeneratorSignalType.Pink
    ...
    def randomNoiseDuration(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.Duration = self.lineEdit_Duration_RandomNoiseTest.text()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)
    ...
    def randomNoiseFreqLines(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.AnalysisFFTSettings.FrequencyLines = self.comboBox_FreqLines_RandomNoiseTest.currentText()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        self.lineEdit_FreqResolution_RandomNoiseTest.setText(
            str(randomNoiseTestSettings.AnalysisFFTSettings.FrequencyResolution))
    ...
    def randomNoiseFFTBlocksize(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.AnalysisFFTSettings.BlockSize = self.comboBox_FFTBlocksize_RandomNoiseTest.currentText()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        self.lineEdit_FreqResolution_RandomNoiseTest.setText(
            str(randomNoiseTestSettings.AnalysisFFTSettings.FrequencyResolution))
    ...
    def randomNoiseOverlap(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.AnalysisFFTSettings.Overlap = self.lineEdit_Overlap_RandomNoiseTest.text()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)
    ...
    def randomNoiseCPBFmin(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.AnalysisCPBSettings.LowFrequency = self.lineEdit_Fmin_CPB_RandomNoiseTest.text()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)

    def randomNoiseCPBFmax(self):
        randomNoiseTestSettings = engine.GetRandomNoiseTestSettings()
        randomNoiseTestSettings.AnalysisCPBSettings.HighFrequency = self.lineEdit_Fmax_CPB_RandomNoiseTest.text()
        engine.SetRandomNoiseTestSettings(randomNoiseTestSettings)
    ...
    def stepSineLevel(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].StepSine.Level = self.lineEdit_Level_StepSineTest.text()
        engine.SetOutputChannels(outputchannels)

    def stepSineFstart(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].StepSine.StartFrequency = self.lineEdit_Fstart_Output_StepSineTest.text()
        engine.SetOutputChannels(outputchannels)

    def stepSineFend(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].StepSine.EndFrequency = self.lineEdit_Fend_Output_StepSineTest.text()
        engine.SetOutputChannels(outputchannels)

    def stepSineResolution(self):
        if self.comboBox_Resolution_StepSineTest.currentText() == "R10":
            outputchannels = engine.GetOutputChannels(False)
            outputchannels[0].StepSine.ResolutionType = StepSineResolutionType.R10
            engine.SetOutputChannels(outputchannels)
        if self.comboBox_Resolution_StepSineTest.currentText() == "R20":
            outputchannels = engine.GetOutputChannels(False)
            outputchannels[0].StepSine.ResolutionType = StepSineResolutionType.R20
            engine.SetOutputChannels(outputchannels)
        if self.comboBox_Resolution_StepSineTest.currentText() == "R40":
            outputchannels = engine.GetOutputChannels(False)
            outputchannels[0].StepSine.ResolutionType = StepSineResolutionType.R40
            engine.SetOutputChannels(outputchannels)
        if self.comboBox_Resolution_StepSineTest.currentText() == "R80":
            outputchannels = engine.GetOutputChannels(False)
            outputchannels[0].StepSine.ResolutionType = StepSineResolutionType.R80
            engine.SetOutputChannels(outputchannels)

    def stepSineMinCycles(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].StepSine.MinCycles = self.lineEdit_MinCycles_StepSineTest.text()
        engine.SetOutputChannels(outputchannels)

    def stepSineMinDuration(self):
        outputchannels = engine.GetOutputChannels(False)
        outputchannels[0].StepSine.MinDuration = self.lineEdit_MinDuration_StepSineTest.text()
        engine.SetOutputChannels(outputchannels)
