...
generator = engine.SetOutputChannel(1, '4227-A', true, 'None', 1, 'V/V', 1, GeneratorSignalType.Sweep);

% Define sweep settings for this output
sweepSettings = engine.SetOutputChannelSweptSineSettings(1, 0.02, 80, 12000, ScanningModeType.Linear, 10, 0, 0, FadeType.Linear, 0.01,0.01);

% Define the arguments for a swept sine test (see user's manual for details)
arguments = sprintf('%s', 'Swept_Sine_Test');
% Execute the task
